...
var recentlyViewed = {
  ...
  getCookieProducts: function(rvCookie, productHandle){
    ...
    if (!rvCookie && productHandle) {
      Cookies.set('recentlyViewed', productHandle, { expires: 30, path: '', domain: '', sameSite: 'None', secure: true });
      rvCookie = Cookies.get('recentlyViewed');
    } else {
      rvCookie = Cookies.get('recentlyViewed');
    }
  ...
  setCookieProducts: function(rvProductArray){
    Cookies.set('recentlyViewed', escape(rvProductArray.join(',')), { expires: 30, path: '', domain: '', sameSite: 'None', secure: true });
  },
    ...
    if (results && results.length > 0) {
      $.each(results, function(index, result) {
         ...
         //}
           ...
           var wcpcartdata = [];
            ...
            for(var j=0;j<wcpcartdata.products.length;j++){
              if(result.id == wcpcartdata.products[j].id){
                  result.price=wcpcartdata.products[j].variants[0].wpd_price;          
                  result.price_min=wcpcartdata.products[j].wpd_price_min;
                  result.price_max=wcpcartdata.products[j].wpd_price_max;
                  result.compare_at_price_max=wcpcartdata.products[j].wpd_compare_at_price_min;
                  result.compare_at_price_min=wcpcartdata.products[j].wpd_compare_at_price_max;
              }
            }       
         //}
        ...
        if (result['price']) {
          ...
          if (result['image']) {
            link.append('<div class="thumbnail"><img class="lazyload transition--' + Shopify.theme_settings.image_loading_style + '" src="' + utils.addImageDimension(result['image'], '_300x') + '" /></div>');
          }
  ...
  productReviewScroll: function(){
    if ($('#shopify-product-reviews').length && $('.shopify-product-reviews-badge').length) {
      $('.spr-badge-container').on('click', function () {
        utils.scrollToTop('#shopify-product-reviews');
      });
    }
  },
...
var quickShop = {
  ...
  afterContent: function($productForm, $notifyForm, productData) {
    ...
    $('.js-quickshop-forms--'+ productData.product_id).empty();
    $productForm.appendTo('.quick-shop__lightbox .js-product-form');
    $notifyForm.appendTo('.quick-shop__lightbox .js-product-form');
  ...
  updateVariant: function(variant){
    ...
    if (globalQuickShopProduct != 'undefined'){
     ...
     var wcpcartdata = [];
      ...
      for(var i=0;i<product.variants.length;i++) { 
          for(var j=0;j<wcpcartdata.products.length;j++){
            for(var x=0;x<wcpcartdata.products[j].variants.length;x++){
              if(product.variants[i].id == wcpcartdata.products[j].variants[x].id){
                  //console.log('===============');
                  product.variants[i].price=wcpcartdata.products[j].variants[x].wpd_price * 100;   
                  product.variants[i].compare_at_price=wcpcartdata.products[j].variants[x]._wpd_compare_at_price;
                  product.variants[i].price_min=wcpcartdata.products[j].wpd_price_min;
                  product.variants[i].price_max=wcpcartdata.products[j].wpd_price_max;
                  product.variants[i].compare_at_price_max=wcpcartdata.products[j].wpd_compare_at_price_min;
                  product.variants[i].compare_at_price_min=wcpcartdata.products[j].wpd_compare_at_price_max;
              }
            }
  ...
  populateGallery: function(productData){
    ...
    function addMainGalleryImages(){
      ...
      $.each( productData.image_array, function( i, image ){
        ...
        if (!image.mediaType || image.mediaType.indexOf("image") >= 0 ) {
          cellContent = '<div class="image__container"><img class="lazyload '+ Shopify.theme_settings.image_loading_style +'" src="' + imgPath + '" alt="${alt}" data-zoom-src="' + imgPath +'" data-image-id="' + image.id + '" data-index="' + i + '" /></div>';
        } else {
...
var newsletter_popup = {
  init: function(){
    ...
    if (cookie_enabled) {
      Cookies.set('popup', 'open', { expires: newsletter_popup_days, path: '', domain: '', sameSite: 'None', secure: true });
    }
  },
  open: function(){
